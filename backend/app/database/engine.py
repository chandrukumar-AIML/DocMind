...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        try:
            ...
            print("   ✅ No password: URL unchanged")
            ...
            print("   ✅ PostgreSQL: asyncpg -> psycopg2")
            ...
            print("   ✅ Session factories: SyncSessionLocal, AsyncSessionLocal")

            # -- Test 4: Session getters (mocked) ----------------------
            print("\n📌 Test 4: Session getters (mocked creation)")

            with patch("app.database.engine.SyncSessionLocal") as mock_sync, patch(
                "app.database.engine.AsyncSessionLocal"
            ) as mock_async:
                ...
                print("   ✅ get_sync_session() returns session")

                async_sess = await get_async_session()
                assert async_sess is not None
                print("   ✅ get_async_session() returns async session")
            ...
            with patch("app.database.engine.async_engine") as mock_engine:
                ...
                print("   ✅ Health check (basic): passed")

                # Test with schema verify
                healthy = await check_database_health(verify_schema=True)
                assert healthy is True
                print("   ✅ Health check (with schema): passed")

                # Test with missing table
                mock_result.scalar.return_value = False
                healthy = await check_database_health(verify_schema=True)
                assert healthy is False
                print("   ✅ Health check (missing table): correctly failed")
            ...
            with patch("app.database.engine.engine", mock_sync_engine), patch(
                "app.database.engine.async_engine", mock_async_engine
            ):
                await dispose_engines()

                mock_sync_engine.dispose.assert_called_once()
                mock_async_engine.dispose.assert_called_once()
                print("   ✅ dispose_engines() called dispose on both mocked engines")
