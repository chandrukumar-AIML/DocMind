...
class MetricsCollector:
    ...
    async def compute_window_stats_async(
        ...
        stats = {
            ...
            "faithfulness_alert": (
                (compute_mean(faithfulness_vals) or 1.0) < 0.70
                if faithfulness_vals else False
            ),
...
def record_query_error(
    ...
    try:
        logger.error(
            f"Query error | workspace={workspace_id} "
            f"error={error_type}: {error_message} | {correlation_id}"
        )
    except Exception as e:
...
def record_ingest_error(
    ...
    try:
        logger.error(
            f"Ingest error | workspace={workspace_id} "
            f"error={error_type}: {error_message} | {correlation_id}"
        )
    except Exception as e:
...
def record_document_operation(
    ...
    try:
        ...
        logger.info(
            ...
            f"user={user_id or 'anon'} status={status} | {correlation_id}"
...
__all__ = [
    ...
    "record_query_latency", "record_query_error",
    "record_ingest_latency", "record_ingest_error",
    "record_auth_attempt",
    ...
    async def run_tests():
        ...
        try:
            from app.monitoring.metrics_collector import (
                QueryMetrics, MetricsCollector,
                _validate_metrics_inputs, get_prometheus_metrics,
                get_monitoring_metadata, METRICS_KEY, DAILY_STATS_KEY, WINDOW_SECONDS,
                record_query_latency, record_auth_attempt, record_document_operation
            )
            import time
            ...
            print(f"   ✅ Meta returns config dict")
            
            # -- Test 2: QueryMetrics dataclass -------------------------
            print("\n📌 Test 2: QueryMetrics dataclass")
            
            metrics = QueryMetrics(
                query_id="q-123", workspace_id="ws-456", timestamp=time.time(),
                latency_ms=250.5, retrieval_count=10, reranked_count=5,
                confidence_score=0.92, relevance_score=0.88, is_grounded=True,
                needs_human_review=False, crag_action="generate", retrieval_mode="hybrid",
                retry_count=0, web_search_used=False, answer_length=500, citation_count=3,
            )
            
            assert metrics.query_id == "q-123"
            print(f"   ✅ QueryMetrics: created with required fields")
            
            d = metrics.to_dict()
            assert d["faithfulness"] == -1.0
            print(f"   ✅ to_dict: None values converted to -1.0 sentinel")
            
            restored = QueryMetrics.from_dict(d)
            assert restored.faithfulness is None
            print(f"   ✅ from_dict: -1.0 sentinel converted back to None")
            ...
            print(f"   ✅ MetricsCollector: initialized with custom Redis URL")
            ...
            with patch('app.monitoring.metrics_collector.get_monitoring_redis') as mock_get_redis:
                ...
                print(f"   ✅ record_async: Redis pipeline executed correctly")
            ...
            with patch('app.monitoring.metrics_collector.get_monitoring_redis') as mock_get_redis:
                ...
                print(f"   ✅ record_async: returns False on Redis failure (graceful degradation)")
            ...
            with patch('app.monitoring.metrics_collector.get_monitoring_redis') as mock_get_redis:
                mock_redis = AsyncMock()
                mock_metrics_json = [
                    '{"query_id": "q1", "workspace_id": "ws1", "timestamp": %s, "latency_ms": 100, "retrieval_count": 5, "reranked_count": 3, "confidence_score": 0.9, "relevance_score": 0.85, "is_grounded": true, "needs_human_review": false, "crag_action": "generate", "retrieval_mode": "vector", "retry_count": 0, "web_search_used": false, "answer_length": 200, "citation_count": 2}' % time.time()
                ]
                ...
                print(f"   ✅ get_recent_async: fetched and parsed metrics")
            ...
            with patch.object(collector, 'get_recent_async') as mock_get_recent:
                mock_get_recent.return_value = [
                    QueryMetrics(
                        query_id=f"q{i}", workspace_id="ws1", timestamp=time.time(),
                        latency_ms=100.0 + i*10, retrieval_count=5, reranked_count=3,
                        confidence_score=0.9, relevance_score=0.85, is_grounded=True,
                        needs_human_review=False, crag_action="generate", retrieval_mode="vector",
                        retry_count=0, web_search_used=(i % 2 == 0), answer_length=200, citation_count=2
                    )
                    for i in range(10)
                ]
            ...
            with patch('app.config.get_settings') as mock_settings:
                ...
                print(f"   ✅ Basic Prometheus: valid format with version & correlation_id")
            
            # -- Test 10: Prometheus metrics (advanced) -----------------
            print("\n📌 Test 10: Prometheus metrics (advanced with stats)")
            
            with patch('app.config.get_settings') as mock_settings, \
                 patch('app.monitoring.metrics_collector.MetricsCollector') as MockCollector, \
                 patch('app.monitoring.metrics_collector.run_async_in_task') as mock_run:
                
                mock_settings.return_value.app_version = "1.0.0"
                mock_settings.return_value.redis_url = "redis://localhost:6379"
                
                mock_stats = {
                    "query_count": 100, "latency_ms_mean": 250.5, "latency_ms_p95": 800.0,
                    "latency_ms_p99": 1200.0, "confidence_mean": 0.92, "relevance_mean": 0.88,
                    "web_search_rate": 0.15, "human_review_rate": 0.05,
                    "crag_action_distribution": {"generate": 80, "rewrite": 15, "web_search": 5},
                }
                ...
                print(f"   ✅ Advanced Prometheus: includes query stats & CRAG distribution")
            ...
            with patch('app.config.get_settings') as mock_settings, \
                ...
                print(f"   ✅ Error fallback: returns valid basic metrics with error label")
            ...
            try:
                ...
                print(f"   ✅ Convenience functions: execute without errors")
            except Exception as e:
