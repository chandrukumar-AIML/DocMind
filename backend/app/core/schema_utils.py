...
def validate_correlation_id(cid: Optional[str]) -> Optional[str]:
    ...
    if not _CORRELATION_ID_PATTERN.match(cid):
        raise ValueError("correlation_id must be 8-100 chars, containing only letters, numbers, hyphens, underscores")
    return cid
...
def dataclass_to_pydantic(dc: Any, model_cls: type[T], exclude: set[str] = None) -> T:
    ...
    data = {k: v for k, v in dc.__dict__.items() if k not in exclude and not k.startswith("_")}
