...
if __name__ == "__main__":
    ...
    def run_tests():
        ...
        try:
            ...
            fused = reciprocal_rank_fusion(ranked_lists=[list_1, list_2], weights=[0.5, 0.5], k=3)
            ...
            context, citations = build_safe_context(documents=[(doc1, 0.9), (doc2, 0.8)], max_docs=2)
            ...
            safe_context, _ = build_safe_context(documents=[(injection_doc, 0.9)], max_docs=1)
            # ✅ FIX: Check for escaped script tag, not missing tag
            assert "\\<script\\>" in safe_context
            print("   ✅ Context content escaped (XSS prevented)")
