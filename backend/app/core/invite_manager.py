...
async def accept_invite(
    ...
    Returns JWT tokens.
    """
    from app.auth.jwt_handler import hash_password, create_access_token, create_refresh_token
    ...
    async with async_engine.begin() as conn:
        ...
        """), {"email": invite["email"]})).mappings().fetchone()
        ...
        if existing:
            ...
            await conn.execute(text("""
                ...
                WHERE id = :uid
            """), {"name": full_name, "pw": hashed_pw,
                   "role": invite["role"], "uid": user_id})
        else:
            ...
            """), {
                "email": invite["email"], "name": full_name,
                "pw": hashed_pw, "role": invite["role"],
            })).mappings().fetchone()
...
async def resend_invite(invite_id: str, invited_by_user_id: str) -> tuple[str, dict[str, Any]]:
    ...
    async with async_engine.connect() as conn:
        ...
        """), {"id": invite_id})).mappings().fetchone()
...
async def get_onboarding_progress(workspace_id: str) -> dict[str, bool]:
    async with async_engine.connect() as conn:
        ...
        """), {"wsid": workspace_id})).scalar() or 0

        query_count = (await conn.execute(text("""
            SELECT COUNT(*) FROM usage_logs
            WHERE workspace_id = :wsid
              AND action_type IN ('query_executed', 'agent_query', 'graph_query')
        """), {"wsid": workspace_id})).scalar() or 0

        key_count = (await conn.execute(text("""
            SELECT COUNT(*) FROM api_keys
            WHERE workspace_id = :wsid AND is_active = TRUE
        """), {"wsid": workspace_id})).scalar() or 0

        member_count = (await conn.execute(text("""
