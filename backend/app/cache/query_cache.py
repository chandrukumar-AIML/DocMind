...
from app.core.serializers import cache_serialize, cache_deserialize, safe_json_loads, safe_json_dumps
...
class QueryCache:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None):
        ...
        try:
            ...
            if "upstash" in redis_url.lower():
                ...
                logger.info(f"QueryCache initialized | mode=redis")
    ...
    def _validate_cache_inputs(self, workspace_id: str, question: Optional[str], result: Optional[dict], corr_id: str) -> tuple[bool, str]:
    ...
    async def _with_retry(self, operation, *args, **kwargs):
        ...
        if self._mode == "memory" or self._redis_failed or not self._redis:
            ...
            return True if operation.__name__ in ['setex', 'eval', 'delete', 'ping', 'hincrby', 'hgetall', 'scan', 'incrby', 'expire', 'exists', 'keys'] else None
    ...
    async def set_result(self, workspace_id: str, question: str, result: dict, filter_dict: Optional[dict] = None, ttl: Optional[int] = None) -> bool:
        """✅ FINAL FIX: Check fallback state after _with_retry"""
        ...
        await self._with_retry(
            self._redis.eval, _SET_RESULT_SCRIPT, 2,
            key, ws_index_key, serialized, target_ttl, index_ttl
        )
    ...
    async def flush_all(self, max_keys: int = 10000) -> tuple[int, bool]:
        ...
        total_deleted = 0
        try:
            for prefix in (CACHE_PREFIX_EMBED, CACHE_PREFIX_RESULT, CACHE_PREFIX_WORKSPACE):
                cursor = 0
                while True:
    ...
    async def exists(self, key: str) -> bool:
        if self._mode == "memory" or self._redis_failed or not self._redis:
            await self._ensure_memory_mode()
            return key in self._memory_store and (key not in self._memory_expiry or time.time() <= self._memory_expiry[key])
...
def get_cache_metadata() -> dict[str, Any]:
    return {
        "prefixes": {"embed": CACHE_PREFIX_EMBED, "result": CACHE_PREFIX_RESULT, "workspace": CACHE_PREFIX_WORKSPACE, "stats": CACHE_PREFIX_STATS},
        "ttl_defaults": {"embed_seconds": DEFAULT_EMBED_TTL_SECONDS, "result_seconds": DEFAULT_RESULT_TTL_SECONDS},
        "min_confidence_to_cache": MIN_CONFIDENCE_TO_CACHE,
        "retry_config": {"max_attempts": REDIS_RETRY_CONFIG.max_attempts, "backoff_base": REDIS_RETRY_CONFIG.backoff_base, "backoff_max": REDIS_RETRY_CONFIG.backoff_max},
    ...
    async def run_tests():
        ...
        try:
            ...
            if still_exists is None:
                print(f"   verify deletion: ✅ PASS | result cleared")
            else:
                print(f"   verify deletion: ❌ FAIL | result still present")
