...
"""
    try:
        ...
        return {
            ...
            "agent_steps": [f"QueryAnalyzer: type={parsed['query_type']} route={parsed['retrieval_route']} | {parsed['reasoning']}"],
        }
...
async def node_relevance_grader(state: AgentState) -> dict:
    ...
    if docs_to_grade:
        doc_snippets = "\n\n".join(
            f"[{i}]: {doc.page_content[:300]}" for i, doc in enumerate(docs_to_grade)
        )
...
{doc_snippets}"""
    ...
    return {
        ...
        "crag_action": "generate",
        "agent_steps": [f"RelevanceGrader[DEPRECATED]: score={aggregate_score:.2f} | {len([g for g in graded_docs if g['relevant']])}/{len(graded_docs)} relevant"],
    }
...
is_grounded = true only if ALL factual claims are supported by context."""
    ...
    try:
        ...
        if hasattr(llm, "with_structured_output"):
            ...
            response = await llm.ainvoke([HumanMessage(content=prompt)])
            raw = _safe_json_parse(response.content, {"is_grounded": True, "unsupported_claims": [], "confidence": 0.8})
        ...
        return {
            ...
            "agent_steps": [f"HallucinationChecker: grounded={is_grounded} | flags={len(flags)} | needs_review={needs_review}"],
            "error": None,
            "error_code": None,
        }
    except Exception as e:
...
async def node_crag_grader(state: AgentState) -> dict:
    ...
    try:
        ...
        graded_docs = [
            {"doc": g.document, "score": g.score, "relevant": g.is_relevant, "reason": g.reason, "label": g.label.value}
        ...
        return {
            ...
            "agent_steps": [f"CRAGGrader: {result.relevant_count}/{len(docs)} relevant | action={result.crag_action} | mean={result.mean_score:.2f}"],
            "error": None,
            "error_code": None,
        }
    except Exception as e:
...
async def node_web_search(state: AgentState) -> dict:
    ...
    try:
        ...
        return {
            ...
            "agent_steps": [f"WebSearch: '{search_query[:50]}' -> {result.result_count} results | total_docs={len(new_docs)}"],
            "error": None,
            "error_code": None,
        }
    except Exception as e:
...
async def node_query_decomposer(state: AgentState) -> dict:
    ...
    try:
        ...
        return {
            ...
            "retry_count": retry_count + 1,
            "agent_steps": [f"QueryDecomposer: '{question[:40]}' -> {len(decomposed.sub_questions)} sub-questions | {decomposed.decomposition_reasoning[:60]}"],
            "error": None,
            "error_code": None,
        }
    except Exception as e:
...
async def node_self_rag_reflector(state: AgentState) -> dict:
    ...
    try:
        ...
        assessment = await reflector.reflect(
            question=question, answer=answer, context_docs=context_docs
        )

        should_retrieve = (
            assessment.retrieve_more
            and retry_count < MAX_SELF_RAG_RETRIES
            and bool(assessment.additional_queries)
        )

        new_question = (
            assessment.additional_queries[0]
            if should_retrieve and assessment.additional_queries
            else question
        )
        ...
        return {
            ...
            "agent_steps": [f"SelfRAG: supported={assessment.is_supported} | complete={assessment.is_complete} | retrieve_more={should_retrieve}"],
            "error": None,
            "error_code": None,
        }
    except Exception as e:
    ...
    async def run_tests():
        ...
        try:
            from app.agent.nodes import (
                _safe_json_parse, _validate_state_keys,
                QueryAnalysisSchema, GradingSchema, HallucinationCheckSchema,
                node_query_analyzer, node_answer_generator, node_hallucination_checker
            )
            ...
            # Schemas
            parsed = QueryAnalysisSchema(query_type="factual", retrieval_route="vector", 
                                         standalone_question="test?", reasoning="test")
            ...
            state = {"question": "What is AI?", "correlation_id": "test-qa", "chat_history": []}
            ...
            with patch('app.agent.nodes.get_llm') as mock_get_llm:
                ...
                mock_llm.with_structured_output.return_value.ainvoke = AsyncMock(
                    return_value=QueryAnalysisSchema(
                        query_type="factual", retrieval_route="vector", 
                        standalone_question="What is artificial intelligence?", reasoning="General query"
                    )
                )
            ...
            graded_docs = [{"doc": MagicMock(page_content="AI is smart.", metadata={"source_file": "doc.pdf", "page_number": 0, "block_type": "p"}), "score": 0.9, "relevant": True}]
            state = {"standalone_question": "What is AI?", "graded_docs": graded_docs, "relevance_score": 0.9, "correlation_id": "test-ag"}
            ...
            state = {
                "answer": "AI is great.", "graded_docs": [{"doc": MagicMock(page_content="AI is artificial intelligence.", metadata={}), "score": 0.8, "relevant": True}],
                "graph_context": "", "correlation_id": "test-hc"
            }
            ...
            assert result["is_grounded"] is True and result["agent_steps"][0].startswith("HallucinationChecker: no-answer")
