...
def state_from_dict(data: dict[str, Any]) -> AgentState:
    ...
    for key in [
        "correlation_id", "chat_history", "filter_dict", "query_type", "retrieval_route",
        "retrieved_docs", "graph_context", "graph_records", "relevance_score", "graded_docs",
        "retry_count", "answer", "citations", "confidence_score", "is_grounded",
        "hallucination_flags", "needs_human_review", "agent_steps", "error", "error_code",
        "crag_action", "missing_info", "web_search_used", "sub_questions", "decomposed_query",
        "self_rag_retrieve_more", "self_rag_confidence", "self_rag_supported",
        "self_rag_complete", "self_rag_notes",
    ]:
        if key in data:
    ...
    def run_tests():
        ...
        try:
            from app.agent.state import (
                AgentState, _sanitize_filter_dict,
                validate_agent_state_minimal, validate_agent_state_strict,
                state_to_dict, state_from_dict, get_required_fields,
                _PYDANTIC_AVAILABLE
            )
            if _PYDANTIC_AVAILABLE:
            ...
            print(f"   ✅ _sanitize_filter_dict: preserves safe values")
            ...
            assert res["safe"] == 1
            print(f"   ✅ _sanitize_filter_dict: removes functions")
            
            # Required fields
            assert get_required_fields() == ["question", "workspace_id", "standalone_question"]
            print(f"   ✅ get_required_fields: returns correct list")

            # -- Test 2: Minimal Validation -----------------------------
            print("\n📌 Test 2: validate_agent_state_minimal")
            
            # Valid
            valid = {"question": "Test?", "workspace_id": "ws-1", "standalone_question": "Test?"}
            is_valid, err = validate_agent_state_minimal(valid)
            assert is_valid is True and err is None
            print(f"   ✅ Minimal: valid state passes")
            
            # Missing keys
            missing = {"workspace_id": "ws-1"}
            is_valid, err = validate_agent_state_minimal(missing)
            assert is_valid is False and "Missing" in err
            print(f"   ✅ Minimal: catches missing keys")
            
            # Invalid score
            bad_score = {"question": "Q", "workspace_id": "W", "standalone_question": "Q", "confidence_score": 1.5}
            is_valid, err = validate_agent_state_minimal(bad_score)
            assert is_valid is False and "0.0" in err
            print(f"   ✅ Minimal: rejects out-of-range scores")
            
            # Empty string
            empty_q = {"question": " ", "workspace_id": "W", "standalone_question": "Q"}
            is_valid, err = validate_agent_state_minimal(empty_q)
            assert is_valid is False
            print(f"   ✅ Minimal: rejects empty strings")

            # -- Test 3: Serialization ----------------------------------
            print("\n📌 Test 3: state_to_dict & state_from_dict")
            
            raw_data = {
                "question": "Test?", "workspace_id": "ws-1", "standalone_question": "Test?",
            ...
            if _PYDANTIC_AVAILABLE:
                ...
                print(f"   ✅ Pydantic: creates valid model & strips strings")
                ...
                print(f"   ✅ Pydantic: clamps scores to [0.0, 1.0] range")
                ...
                print(f"   ✅ Pydantic: sanitizes filter_dict automatically")
                ...
                print(f"   ✅ Strict validation: passes valid dict")
                
                # Test rejection of short question (min_length=3)
                invalid_short_q = {"question": "A", "workspace_id": "W", "standalone_question": "Test"}
                is_valid, err = validate_agent_state_strict(invalid_short_q)
                assert is_valid is False
                print(f"   ✅ Strict validation: rejects short question (< 3 chars)")
            
            else:
                print(f"   ⏭️ Skipped: Pydantic not available (falling back to minimal validation)")
                # Fallback test
                is_valid, err = validate_agent_state_strict({"question": "Test", "workspace_id": "W", "standalone_question": "Test"})
                assert is_valid is True
