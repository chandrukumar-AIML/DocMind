    ...
    def run_tests():
        ...
        try:
            from app.agent.edges import (
                route_after_analysis, route_after_grading, route_after_retry,
                route_after_hallucination_check, route_after_crag_grading,
                route_after_self_rag, route_after_web_search, route_after_decomposer,
                _validate_route_target, get_routing_metadata, _MAX_RETRIES
            )
            ...
            # Valid target
            result = _validate_route_target("vector_retriever", {"vector_retriever", "graph_retriever"}, "test", "ctx")
            assert result == "vector_retriever"
            print(f"   ✅ Valid target: returned as-is")
            
            # Invalid target -> fallback to first allowed
            with patch('app.agent.edges.logger') as mock_logger:
                result = _validate_route_target("invalid_route", {"vector_retriever", "graph_retriever"}, "test", "ctx")
                assert result in {"vector_retriever", "graph_retriever"}
                assert mock_logger.error.called
                print(f"   ✅ Invalid target: logged error + returned fallback")
            ...
            print(f"   ✅ Vector route: returns 'vector_retriever'")
            
            # Missing retrieval_route -> default to vector
            state = {"correlation_id": "test-4"}
            assert route_after_analysis(state) == "vector_retriever"
            print(f"   ✅ Missing route: defaults to 'vector_retriever'")
            ...
            print(f"   ✅ Graph retry: returns 'graph_retriever'")
            ...
            print(f"   ✅ Vector retry: returns 'vector_retriever'")
            ...
            print(f"   ✅ Needs review: routes to 'human_review'")
            ...
            print(f"   ✅ No review: routes to 'end'")
            
            # Missing flag -> default to end
            state = {"correlation_id": "test-13"}
            assert route_after_hallucination_check(state) == "end"
            print(f"   ✅ Missing flag: defaults to 'end'")
            ...
            print(f"   ✅ Generate: routes to 'answer_generator'")
            
            # Filter and supplement -> web_search
            state = {"correlation_id": "test-15", "crag_action": "filter_and_supplement"}
            assert route_after_crag_grading(state) == "web_search"
            print(f"   ✅ Filter+supplement: routes to 'web_search'")
            
            # Rewrite with retries left -> query_rewriter
            state = {"correlation_id": "test-16", "crag_action": "rewrite", "retry_count": 0}
            assert route_after_crag_grading(state) == "query_rewriter"
            print(f"   ✅ Rewrite (retries left): routes to 'query_rewriter'")
            
            # Rewrite with retries exhausted -> web_search fallback
            state = {"correlation_id": "test-17", "crag_action": "rewrite", "retry_count": _MAX_RETRIES}
            assert route_after_crag_grading(state) == "web_search"
            print(f"   ✅ Rewrite (no retries): fallback to 'web_search'")
            ...
            print(f"   ✅ Retrieve more: routes to 'vector_retriever'")
            ...
            print(f"   ✅ No more retrieval: routes to 'hallucination_checker'")
            
            # Missing flag -> default to hallucination_checker
            state = {"correlation_id": "test-23"}
            assert route_after_self_rag(state) == "hallucination_checker"
            print(f"   ✅ Missing flag: defaults to 'hallucination_checker'")
            ...
            print(f"   ✅ Web search: always routes to 'answer_generator'")
            ...
            print(f"   ✅ Decomposer: always routes to 'vector_retriever'")
            
            # -- Test 11: Correlation ID propagation in logs ------------
            print("\n📌 Test 11: Correlation ID propagation in logs")
            
            with patch('app.agent.edges.logger') as mock_logger:
                state = {"correlation_id": "custom-corr-123", "retrieval_route": "graph"}
                ...
                print(f"   ✅ Correlation ID 'custom-corr-123' propagated to logs")
            ...
            assert route_after_analysis(empty_state) in {"vector_retriever", "graph_retriever", "hybrid_retrieve_start"}
            assert route_after_grading(empty_state) in {"graph_retriever", "crag_grader"}
            assert route_after_retry(empty_state) in {"vector_retriever", "graph_retriever"}
            assert route_after_hallucination_check(empty_state) in {"human_review", "end"}
            assert route_after_crag_grading(empty_state) in {"answer_generator", "web_search", "query_rewriter", "query_decomposer"}
            assert route_after_self_rag(empty_state) in {"vector_retriever", "hallucination_checker"}
            assert route_after_web_search(empty_state) == "answer_generator"
            assert route_after_decomposer(empty_state) == "vector_retriever"
            
            print(f"   ✅ All routing functions: handle empty state gracefully")
