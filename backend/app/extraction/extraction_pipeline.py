...
from typing import Any, Final, Optional, TYPE_CHECKING, Union, List
...
class ExtractionBundle:
    ...
    @property
    def chart_count(self) -> int: return len(self.charts)
    @property
    def form_count(self) -> int: return len(self.forms)
...
class ExtractionPipeline:
    ...
    async def process_enriched_document_async(
        ...
        logger.info(
            f"[{corr_id}] Extraction complete: {bundle.table_count}T, "
            f"{bundle.chart_count}C, {bundle.form_count}F"
        )
    ...
    async def _process_table_block(
        self, block, source_file: str, counter: int, corr_id: str, timeout: float
    ):
            ...
            try:
                return await asyncio.wait_for(
                    self._table_extractor.extract_async(
                        html=html, text=text, table_id=table_id, 
                        source_file=source_file, page_number=page_num,
                        chunk_id=chunk_id, correlation_id=corr_id
                    ),
                    timeout=timeout
                )
    ...
    async def _process_figure_block(
        self, block, page_img: "np.ndarray", source_file: str, counter: int, corr_id: str, timeout: float  # type: ignore[reportInvalidTypeForm]
    ):
        async with self.semaphore:
            ...
            try:
                return await asyncio.wait_for(
                    self._chart_extractor.extract_from_image_async(
                        image=cropped, chart_id=chart_id,
                        source_file=source_file, page_number=page_num,
                        chunk_id=chunk_id, correlation_id=corr_id
                    ),
                    timeout=timeout
    ...
    def process_enriched_document(
        ...
        try:
            ...
            logger.warning(
                f"⚠️ ExtractionPipeline.process_enriched_document() called from async context — "
                f"use process_enriched_document_async() instead. Returning empty bundle."
            )
            ...
            return asyncio.run(
                self.process_enriched_document_async(enriched, page_images, correlation_id)
            )
