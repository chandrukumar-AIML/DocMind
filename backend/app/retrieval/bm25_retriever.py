...
class BM25Retriever:
    ...
    def _tokenize(self, text: str) -> list[str]:
        ...
        if self.use_unicode:
            ...
            tokens = re.findall(r'\b[\w]{' + str(_MIN_TOKEN_LENGTH) + r',}\b', text.lower(), flags=re.UNICODE)
            return tokens
        else:
    ...
    def search(
        ...
        if max_age_seconds and self._indexed_at:
            ...
            if age > max_age_seconds:
                logger.warning(
                    f"[{corr_id}] BM25 index stale: {age:.0f}s > {max_age_seconds}s — consider rebuilding"
                )
