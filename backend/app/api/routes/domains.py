            ...
            "analysis": {
                "clauses": {
                    ...
                    "items": [
                        ...
                        for c in getattr(extraction, "clauses", [])
                        if c is not None
                    ],
                },
                "risk": {
                    ...
                    "clause_reports": [
                        ...
                        for r in getattr(risk_report, "clause_reports", [])
                        if r is not None
                    ],
                },
                "obligations": [
...
async def analyze_medical_document(
    ...
    try:
        ...
        for chunk in chunks:
            try:
                result = await asyncio.wait_for(
                    asyncio.to_thread(lambda c=chunk: redactor.redact(c.page_content, use_llm_pass=False)),
                    timeout=_EXECUTOR_TIMEOUT,
                )
        ...
        icd_codes = await asyncio.wait_for(
            asyncio.to_thread(lambda: icd_extractor.extract(redacted_chunks, source_file)),
            timeout=_EXECUTOR_TIMEOUT,
        )
        ...
        drug_result = await asyncio.wait_for(
            asyncio.to_thread(lambda: drug_checker.check(redacted_chunks)),
            timeout=_EXECUTOR_TIMEOUT,
        )
        ...
        return {
            ...
            "analysis": {
                ...
                "interactions": [
                    ...
                    for i in getattr(drug_result, "interactions", [])
                    if i is not None
                ],
                "interaction_summary": {
    ...
    for source_file in request.source_files[:20]:  # Cap at 20 invoices
        try:
            ...
            invoice = await asyncio.wait_for(
                asyncio.to_thread(lambda sf=source_file: extractor.extract(chunks, sf)),
                timeout=_EXECUTOR_TIMEOUT,
            )
...
async def extract_invoice(
    ...
    try:
        ...
        extractor = InvoiceExtractor()
        invoice = await asyncio.wait_for(
            asyncio.to_thread(lambda: extractor.extract(chunks, source_file)),
            timeout=_EXECUTOR_TIMEOUT,
        )
...
async def detect_signatures(
    ...
    try:
        ...
        figure_chunks = [
            c for c in chunks if hasattr(c, "metadata") and c.metadata.get("block_type") in ("figure", "image")
        ]
