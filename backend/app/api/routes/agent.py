...
async def agent_query(
    ...
    response_type, response_data = await _execute_agent_query(
        ...
        user=user,
        chat_history=[msg.model_dump() for msg in getattr(body, "chat_history", [])] if getattr(body, "chat_history", None) else None,
        filter_dict=filter_dict,
        stream=stream,
        correlation_id=corr_id,
        request=request,
...
def get_agent_metadata() -> dict[str, Any]:
    ...
    return {
        "endpoints": ["/agent/query", "/agent/thread/{thread_id}", "/agent/confidence/{thread_id}"],
