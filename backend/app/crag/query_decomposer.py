...
"""
    ...
    def _deduplicate_questions(self, questions: list[str]) -> list[str]:
        ...
        for q in questions:
            ...
            if any(normalized.startswith(existing[:20]) or existing.startswith(normalized[:20]) 
    ...
    async def run_tests():
        ...
        try:
            from app.crag.query_decomposer import (
                DecompositionResponseSchema, DecomposedQuery, QueryDecomposer,
                _MIN_SUBQUESTION_LENGTH, _MAX_SUBQUESTIONS, _MAX_QUESTION_LENGTH,
                _DECOMPOSE_RETRY_CONFIG
            )
            ...
            try:
                ...
                print(f"   ❌ Should reject >4 sub-questions")
            except Exception:
                print(f"   ✅ Schema: rejects >4 sub-questions")
            
            # Empty sub-questions -> Pydantic error
            try:
                DecompositionResponseSchema(sub_questions=[], reasoning="test")
                print(f"   ❌ Should reject empty list")
            except Exception:
                print(f"   ✅ Schema: rejects empty sub_questions")
            ...
            print(f"   ✅ DecomposedQuery: properties computed correctly")
            
            # Simple query (1 sub-question)
            simple = DecomposedQuery(
                original="What is X?",
                sub_questions=["What is X?"],
                is_decomposed=False
            )
            assert simple.is_simple is True
            print(f"   ✅ DecomposedQuery: is_simple = True for single question")
            ...
            try:
                ...
                print(f"   ❌ Should reject short sub-question")
            except ValueError:
            ...
            try:
                ...
                print(f"   ❌ Should reject long sub-question")
            except ValueError:
            ...
            print(f"   ✅ Dedup: case-insensitive deduplication works")
            ...
            print(f"   ✅ Dedup: prefix similarity filter works")
            ...
            with patch('app.crag.query_decomposer.get_settings') as mock_settings:
                ...
                with patch('app.crag.query_decomposer.ChatOpenAI') as MockLLM:
                    ...
                    print(f"   ✅ QueryDecomposer: initializes with LLM & structured output flag")
            
            # -- Test 5: decompose() with mocked LLM success -----------
            print("\n📌 Test 5: decompose() (mocked LLM success)")
            
            with patch('app.crag.query_decomposer.get_settings') as mock_settings, \
                 patch('app.crag.query_decomposer.ChatOpenAI') as MockLLM, \
                 patch('app.crag.query_decomposer.retry_async') as mock_retry:
                ...
                mock_retry.return_value = AsyncMock(return_value=DecompositionResponseSchema(
                    sub_questions=["What is X?", "How does Y work?"],
                ...
                print(f"   ✅ decompose(): returns DecomposedQuery with correct sub-questions")
            ...
            with patch('app.crag.query_decomposer.get_settings') as mock_settings, \
                 patch('app.crag.query_decomposer.ChatOpenAI') as MockLLM, \
                 patch('app.crag.query_decomposer.logger') as mock_logger:
                ...
                print(f"   ✅ decompose(): fallback to original query on JSON parse error")
            ...
            with patch('app.crag.query_decomposer.get_settings') as mock_settings, \
                 patch('app.crag.query_decomposer.ChatOpenAI') as MockLLM, \
                 patch('app.crag.query_decomposer.estimate_tokens_approx') as mock_estimate, \
                 patch('app.crag.query_decomposer.build_safe_prompt') as mock_build:
                ...
                mock_structured.ainvoke = AsyncMock(return_value=DecompositionResponseSchema(
                    sub_questions=["Q1"], reasoning="test"
                ))
                ...
                print(f"   ✅ decompose(): handles token safety with truncation")
