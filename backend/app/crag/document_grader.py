...
from app.core.prompts import escape_prompt_content, estimate_tokens_approx, build_grading_prompt
    ...
    async def grade_documents(
        ...
        for batch_start in range(0, len(documents), batch_size):
            ...
            try:
                ...
                for doc in batch:
                    all_grades.append(DocumentGrade(
                        document=doc,
    ...
    async def _grade_batch(
        ...
        for i, doc in enumerate(batch):
            ...
            if estimate_tokens_approx("\n\n".join(doc_snippets) + self.GRADING_PROMPT_TEMPLATE) > 6500:
                logger.warning(f"[{corr_id}] Batch prompt approaching token limit — truncating to {len(doc_snippets)} docs")
                break
    ...
    async def run_tests():
        ...
        try:
            from app.crag.document_grader import (
                GradeLabel, GradeItemSchema, GradingResponseSchema,
                DocumentGrade, GradingResult, DocumentGrader,
                _MAX_DOC_CONTENT_CHARS, _GRADING_RETRY_CONFIG
            )
            from langchain_core.documents import Document
            ...
            print(f"   ✅ GradeLabel: validates correctly")
            
            item = GradeItemSchema(doc_index=0, label="relevant", score=0.9, reason="test")
            assert item.doc_index == 0 and item.label == "relevant"
            print(f"   ✅ GradeItemSchema: validates correctly")
            
            response = GradingResponseSchema(grades=[item])
            assert len(response.grades) == 1
            print(f"   ✅ GradingResponseSchema: validates correctly")
            
            # -- Test 2: DocumentGrade dataclass ------------------------
            print("\n📌 Test 2: DocumentGrade properties")
            doc = Document(page_content="Test content", metadata={"source_file": "test.pdf", "page_number": 1, "chunk_id": "c1"})
            grade = DocumentGrade(
                document=doc, label=GradeLabel.RELEVANT, score=0.9, 
                reason="test reason", missing_info="missing X",
                chunk_id=doc.metadata.get("chunk_id", "")
            )
            ...
            print(f"   ✅ DocumentGrade: properties computed correctly")
            ...
            print(f"   ✅ GradingResult: aggregates counts & mean correctly")
            print(f"   ✅ GradingResult: CRAG action = 'decompose' (50% rel + ambiguous)")
            ...
            print(f"   ✅ GradingResult: CRAG action = 'filter_and_supplement' (when ambiguous=0)")
            ...
            print(f"   ✅ GradingResult: empty grades -> 'rewrite' (Correct CRAG behavior)")
            ...
            with patch('app.crag.document_grader.get_settings') as mock_settings:
                ...
                with patch('app.crag.document_grader.ChatOpenAI') as MockLLM:
                    ...
                    print(f"   ✅ DocumentGrader: initializes with LLM & structured output flag")
            
            # -- Test 5: grade_documents (mocked LLM success) -----------
            print("\n📌 Test 5: grade_documents (mocked LLM success)")
            with patch('app.crag.document_grader.get_settings') as mock_settings, \
                 patch('app.crag.document_grader.ChatOpenAI') as MockLLM:
                mock_settings.return_value.openai_api_key = "test-key"
                mock_llm = MagicMock()
                MockLLM.return_value = mock_llm
                mock_structured = MagicMock()
                mock_structured.ainvoke = AsyncMock(return_value=GradingResponseSchema(
                    grades=[GradeItemSchema(doc_index=0, label="relevant", score=0.9, reason="test")]
                ))
                mock_llm.with_structured_output.return_value = mock_structured
                grader = DocumentGrader()
                docs = [Document(page_content="Test doc", metadata={"source_file": "t.pdf", "page_number": 1, "chunk_id": "c1"})]
                ...
                print(f"   ✅ grade_documents: returns GradingResult with correct grades")
            
            # -- Test 6: grade_documents error fallback -----------------
            print("\n📌 Test 6: grade_documents fallback on LLM error")
            with patch('app.crag.document_grader.get_settings') as mock_settings, \
                 patch('app.crag.document_grader.ChatOpenAI') as MockLLM, \
                 patch('app.crag.document_grader.logger') as mock_logger:
                ...
                print(f"   ✅ grade_documents: fallback to RELEVANT on LLM error (conservative)")
                       
            # -- Test 7: GradingResult helpers --------------------------
            print("\n📌 Test 7: GradingResult helper methods")
            grades = [
                DocumentGrade(Document(page_content="A", metadata={"chunk_id": "c1"}), GradeLabel.RELEVANT, 0.9, "r1", missing_info="missing A"),
                DocumentGrade(Document(page_content="B", metadata={"chunk_id": "c2"}), GradeLabel.IRRELEVANT, 0.2, "i1", missing_info="missing B"),
            ]
            ...
            print(f"   ✅ GradingResult.relevant_docs: returns filtered list")
            
            missing = result.missing_info_summary
            assert "missing A" in missing and "missing B" in missing
            print(f"   ✅ GradingResult.missing_info_summary: aggregates missing info")
            
            d = result.to_dict()
            assert d["total"] == 2 and d["relevant"] == 1
            assert d["crag_action"] in {"generate", "filter_and_supplement", "rewrite", "decompose"}
            print(f"   ✅ GradingResult.to_dict: returns serializable dict")
