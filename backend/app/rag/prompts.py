...
__all__ = [
    ...
    "CONDENSE_QUESTION_SYSTEM_PROMPT", "CONDENSE_QUESTION_PROMPT",
    "ANSWER_SYSTEM_PROMPT", "ANSWER_PROMPT",
    "format_citation", "truncate_for_context", "safe_format_prompt",
...
if __name__ == "__main__":
    ...
    def run_tests():
        ...
        try:
            ...
            print(f"   [OK] ANSWER_SYSTEM_PROMPT: security warnings present")
            
            assert isinstance(FALLBACK_ANSWER_TEMPLATE, str) and "OpenAI unavailable" in FALLBACK_ANSWER_TEMPLATE
            print(f"   [OK] FALLBACK_ANSWER_TEMPLATE: fallback message present")
            ...
            print(f"   [OK] Page numbering: 0-indexed input -> 1-indexed display")
            ...
            print(f"   [OK] Short text: preserved")
            
            long = "A" * 600
            truncated = truncate_for_context(long, max_chars=500)
            assert len(truncated) == 500
            assert truncated.endswith("...")
            print(f"   [OK] Long text: truncated to 500 chars with ellipsis")
            
            exact = "B" * 500
            assert truncate_for_context(exact, max_chars=500) == exact
            print(f"   [OK] Exact length: preserved without ellipsis")
            ...
            print(f"   [OK] Safe input: formatted correctly")
            ...
            print(f"   [OK] Syntax Injection: Special characters & quotes escaped safely")
            
            # Verify template structure preserved
            assert "User asked:" in result and "Context:" in result
            print(f"   [OK] Template structure: preserved after escaping")
            ...
            print(f"   [OK] HYDE_PROMPT: manual format works")
