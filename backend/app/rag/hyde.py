...
class HyDEExpander:
    ...
    def expand(self, query: str, document_context: Optional[str] = None) -> str:
        ...
        try:
            ...
            return asyncio.run_coroutine_threadsafe(
                self.expand_async(query, document_context),
                loop
            ).result()
        except RuntimeError:
    ...
    async def expand_multi_embedding_async(
        ...
        hypotheses = await asyncio.gather(*[
            self.expand_async(query, correlation_id=corr_id) for _ in range(n)
        ])
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        try:
            ...
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                 patch('app.rag.hyde.get_settings') as mock_settings:
                ...
                print(f"   ✅ Initialization: params set correctly")
            ...
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                ...
                print(f"   ✅ LLM called with system + user prompts")
                
                # Verify result
                assert "hypothetical answer" in result.lower()
                print(f"   ✅ Async expand: returned LLM response")
            ...
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                ...
                print(f"   ✅ Query escaped before LLM call")
            ...
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                ...
                print(f"   ✅ Generic LLM error: returns original query")
            
            # Test 5b: Quota error -> uses local fallback (special case)
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                 patch('app.rag.hyde.get_settings') as mock_settings, \
                 patch('app.rag.hyde.is_insufficient_quota_error') as mock_quota_check:
                ...
                assert "definition of" in result.lower()
                print(f"   ✅ Quota error: triggers local fallback expansion")
            ...
            print(f"   ✅ Sync wrapper: method exists and is sync (not async)")
            ...
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                ...
                avg_vector = await expander.expand_multi_embedding_async(
                    "What is AI?",
                    embed_fn=mock_embed,
                    n=3
                )
            ...
            with patch('app.rag.hyde.get_llm') as mock_get_llm, \
                 patch('app.rag.hyde.get_settings') as mock_settings, \
                 patch('app.rag.hyde.logger') as mock_logger:
                ...
                log_calls = [str(call) for call in mock_logger.info.call_args_list + mock_logger.debug.call_args_list + mock_logger.warning.call_args_list]
                assert any(custom_corr_id in call for call in log_calls), f"Correlation ID not found in logs: {log_calls}"
