...
from app.observability.mlflow_logger import MLflowLogger, configure_mlflow
from app.observability.langsmith_config import get_run_metadata, configure_langsmith

def test_mlflow_uri_fix():
    """Verify start_run() doesn't crash with undefined uri."""
    with patch("app.observability.mlflow_logger.mlflow") as mock_mlflow, \
         patch("app.observability.mlflow_logger.get_settings") as mock_settings:
...
def test_langsmith_metadata_truncation():
    ...
    metadata = get_run_metadata(
        extra={"long_value": "x" * 1000, "short": "ok"}
    )
