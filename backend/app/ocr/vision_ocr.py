...
from app.core.ocr_utils import scrub_pii_for_ocr, normalize_bbox, generate_ocr_correlation_id
...
class VisionOCREngine:
    ...
    def __init__(
        ...
        logger.info(
            f"VisionOCR initialized: model={model}, timeout={timeout_seconds}s, "
            f"cost_tracking={track_costs}"
        )
    ...
    def _process_page_sync(
        ...
        user_content = [
            ...
            {"type": "text", "text": f"Extract all text. {'Context: ' + context if context else ''} Return structured JSON."},
        ]
    ...
    async def run_tests():
        ...
        try:
            from app.ocr.vision_ocr import (
                VisionOCREngine, VisionOCRMetrics, VisionBlockSchema, VisionResponseSchema,
                _VISION_COST_PER_1K_TOKENS, _AVG_TOKENS_PER_PAGE
            )
            from app.ocr.paddle_ocr import TextBlock, PageOCRResult
            
            # -- Test 1: Module imports & dataclasses ---------------------
            print("\n📌 Test 1: Module imports & dataclass validation")
            
            metrics = VisionOCRMetrics(
                page_num=0, tokens_used=2000, estimated_cost=0.02, latency_ms=1500.5, correlation_id="test-123"
            )
            ...
            try:
                ...
                print(f"   ✅ VisionOCRMetrics is immutable (frozen)")
            ...
            valid_json = json.dumps({
                "blocks": [
                    {"text": "Invoice #12345", "block_type": "title", "confidence": 0.98, "reading_order": 0, "bbox": [[10, 10], [200, 10], [200, 40], [10, 40]], "language": "en"},
                    {"text": "Total: $1,234.56", "block_type": "paragraph", "confidence": 0.95, "reading_order": 1, "bbox": [[10, 50], [150, 50], [150, 70], [10, 70]]}
                ]
            })
            ...
            assert empty_blocks == []
            print(f"   ✅ Invalid JSON handled: returned empty list")
            ...
            print(f"   ✅ Missing bbox: placeholder used")
            ...
            with patch("app.ocr.vision_ocr.OpenAI") as mock_openai:
                ...
                mock_response.choices[0].message.content = json.dumps({
                    "blocks": [{"text": "Mock Vision OCR", "confidence": 0.92, "bbox": [[0,0],[100,0],[100,20],[0,20]], "reading_order": 0}]
                })
                ...
                with patch("app.ocr.vision_ocr.image_to_b64", return_value="iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg=="):
            ...
            print(f"   ✅ PII scrubbing: sensitive data masked in text content")
