...
class DocumentPreprocessor:
        ...
        try:
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    None,
                    lambda: self.preprocess(image_input, corr_id)
                ),
            ...
            fallback = np.zeros((h, w, 3), dtype=np.uint8) if isinstance(image_input, np.ndarray) and image_input.ndim == 3 else np.zeros((h, w), dtype=np.uint8)
    ...
    def _deskew_safe(self, img: np.ndarray, corr_id: str) -> tuple[np.ndarray, float]:
        ...
        try:
            ...
            if len(coords) < _MIN_DESKEW_POINTS:
                logger.debug(f"[{corr_id}] Skipping deskew: insufficient text regions ({len(coords)} < {_MIN_DESKEW_POINTS})")
                return img, 0.0
    ...
    def _guard_image_size(self, img: np.ndarray, corr_id: str) -> np.ndarray:
        ...
        if max(h, w) > _MAX_IMAGE_DIM:
            ...
            logger.warning(
                f"[{corr_id}] Image too large ({w}x{h} > {_MAX_IMAGE_DIM}) — downscaling to {new_size}"
            )
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        try:
            ...
            print(f"   ✅ Initialized: dpi={preprocessor.target_dpi}, no normalization={not pipeline_info['normalization_applied']}")
            ...
            assert result.image.shape[:2] == (1200, 600), f"Expected (1200, 600), got {result.image.shape[:2]}"
