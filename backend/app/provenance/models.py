...
from sqlalchemy import (
    ...
    ForeignKey, Integer, String, Text, Index,
...
class Answer(Base):
    ...
    updated_at = Column(DateTime(timezone=True), default=lambda: utcnow(), onupdate=lambda: utcnow(), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint("confidence_score IS NULL OR (confidence_score >= 0.0 AND confidence_score <= 1.0)", name="ck_answer_confidence_range"),
    )
...
class Citation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("confidence_score >= 0.0 AND confidence_score <= 1.0", name="ck_citation_confidence_range"),
    )
